...
void levelOrderCreateBiTree2(TreeNode*& BT, vector<int>& IntVec, vector<int>& IsNullVec)
{
...
void UnitTest7()
{
	...
	strcpy(theIniPath, "../Test_Ini/MaxDepth.ini");
	...
	for (int index = 1; index <= theNum; index++)
	{
		...
		// 构建二叉树
		TreeNode* BT;
		levelOrderCreateBiTree2(BT, IntVec, IsNullVec);	// 构建的数是BT

		int result = MaxDepth(BT);
